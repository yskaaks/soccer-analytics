...
def main(config):
    ...
    # Works for images with a resolution of 1080p
    guess_fx = 2000
    
    guess_rot = np.array([[0.25, 0, 0]])
    guess_trans = (0, 0, 80)
    
    K, to_device_from_world, rot, trans, image_rgb = calibrate_from_image(image_rgb,
                                                                          guess_fx,
                                                                          guess_rot,
                                                                          guess_trans)
    
    
    # Is it necessary to recalculate the keypoints?
    key_points, key_lines = find_key_points(image_rgb)
    key_points_array = key_points.compute_points_array()
    
    print("\n---------Recomputed Keypoints:")
    print(key_points)
    
    print("\n---------Keypoints as array:")
    print(key_points_array)
    
    
    key_points_layout = np.load(config['input_layout_array'])
    print("\n---------Keypoints of Layout:")
    print(key_points_layout)
    
    H, key_points_filtered = compute_homography_matrix(key_points_array, key_points_layout)
    print("\n---------Homography matrix:")
    print(H)
    ...
    print("\n---------Points conversion:")
    print(key_points_filtered[1])
    print(apply_homography_to_point(H, key_points_filtered[1]))
    
    print("\n---------Visualize new points:")
    key_points_transformed = apply_homography_to_array(H, key_points_filtered)
    im_out = draw_points_on_image(im_out, key_points_transformed)
    
    # Visualization and extra stuff -------------------------------------------
    image_rgb = key_points.draw(image_rgb)
    
    if to_device_from_world is not None:
        image_rgb = draw_pitch_lines(K, to_device_from_world, image_rgb)
        image_rgb = display_yaw_and_focal_length(
            image_rgb, guess_rot[0, 1] * 180 / np.pi, K[0, 0]
        )
...
if __name__ == "__main__":
    config = {'input_image': '../../../../Datasets/soccer_field_homography/test_9.png',
