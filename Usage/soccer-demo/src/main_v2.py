...
class DetectionProcessor:
    ...
    def compute_detected_objects(self, yolo_detections, frame):
        ...
        for lbl, det_xyxy in zip(labels, detections_as_xyxy):
            ...
            det_object = DetectedObject(det_xyxy, lbl, lbl)
    ...
    def draw_transformed_points_with_heatmap(self, original_frame, detected_objects, heatmap):
        ...
        for detected_obj in detected_objects:
            ...
            id_index = detected_obj.id - 2  # Adjust id to be 0-based index
...
class VideoProcessor:
    def __init__(self, config, H=None):
        self.config = config
        self.detection_processor = DetectionProcessor(config["yolo_model_path"])
        self.H = H
...
if __name__ == "__main__":
    ...
    if H is not None:
        processor = VideoProcessor(config, H)
